...
public static class Models
{
        ...
        List<PropertiesObject>? multipleProperties, int indent = 1)
    {
        ...
        if (multipleProperties != null)
        {
            ...
            var uniqueProperties = Tools.CompareProperties(multipleProperties);
            foreach (var property in multipleProperties)
                if (property.Properties != null)
                    uniqueProperties = uniqueProperties.Merge(property.Properties);

            Tools.GenerateProperties(multiplePropertiesSB, new StringBuilder(), uniqueProperties, interfaceName,
                indent);
        ...
        sb.AppendLineWithIndent("{", indent);
        if (properties != null) sb.Append(propertiesSB);
        if (multipleProperties != null)
        {
            var multiplePropertiesSB = new StringBuilder();
            var
                uniqueProperties =
                    Tools.CompareProperties(multipleProperties); //new Dictionary<string, PropertiesObject>();
            ...
            foreach (var property in multipleProperties)
            {
                ...
                if (property.Properties != null) uniqueProperties = uniqueProperties.Merge(property.Properties);
            }
