...
namespace Generator.Generators
{
    public class Controls
    {
        ...
        public static void GenerateArgs(StringBuilder sb, StringBuilder subSB, string path, PathsObject.RequestBodyClass requestBody)
        {
            ...
            if (properties != null)
            {
                ...
                {
                    string schema = Tools.ConvertToPascalCase(path.Replace("/","-")) + "Properties";
                ...
                foreach (var (key, property) in defaultValues)
                {
                    if (property.ObjectType != null)
                    {
                        ...
                        else if (property.Default != null)
                        {
                            ...
                            {
                                argSB.Append($"{property.ObjectType.Value.Type}.{Tools.ConvertToPascalCase(property.Default.ToString())},");
                            }
        ...
        public static Tools.ObjectType GetType(StringBuilder sb, string schema, string propertyName, PropertiesObject property, int indent)
        {
            var type = Tools.ParseObjectType(property.Type, property.Format);
            var nullable = type.IsNullable;
            if (type.Type == "object")
            {
                type = Tools.GenerateObjectType(sb, sb, schema, propertyName, property, property.SourceInterfaceName != null, indent);
            }
            else if (type.Type == "array" && property.Items?.Type != null)
            {
                type = Tools.GenerateArrayType(sb, sb, schema, propertyName, property.Items, property.SourceInterfaceName != null, indent);
            }
        ...
        public static void GenerateFunction(StringBuilder sb, string path, PathsObject pathObject, ApiJsonModels.HttpMethod httpMethod, int indent = 2)
        {
            var responseType = new Tools.ObjectType();
            var propertiesSB = new StringBuilder();
            var argsSB = new StringBuilder();
            bool needToken = pathObject.Security != null;
            bool useForm = pathObject.RequestBody?.Content.ContainsKey("multipart/form-data") ?? false;
            bool useGet = httpMethod == ApiJsonModels.HttpMethod.Get;
            bool needParam = pathObject.RequestBody != null;
            bool foundResponse = pathObject.Responses?.ContainsKey(200) ?? false;
            string[] id = pathObject.OperationId.Split("___");
            string schema = Tools.ConvertToPascalCase(id[0]);
            string functionName = Tools.ConvertToPascalCase(id[^1]);
            string requestType = useForm ? "multipart/form-data" : "application/json";
            if (useGet)
            {
            ...
            if (needParam)
            {
                GenerateArgs(argsSB, propertiesSB, path, pathObject.RequestBody!);
            }
            if (foundResponse)
            {
                PropertiesObject? res = pathObject.Responses?[200].Content["application/json"].Schema;
            ...
            if (needParam)
            {
                ...
                {
                    GenerateParamDictionaryCode(sb, pathObject.RequestBody?.Content[requestType].Schema.Properties.Keys.ToList(), indent + 1);
                }
            }
            ...
            if (needParam)
            {
                sb.AppendLineWithIndent("param, ", indent + 2);
            }
            if (!foundResponse)
            {
        ...
        public static void Generate(Dictionary<string, Dictionary<ApiJsonModels.HttpMethod, PathsObject>> paths)
        {
            ...
            foreach (var (p, pathsObject) in paths)
            {
                ...
                // ignore paths
                var ignorePaths = new string[] { "drive/files/check-existence", "i/2fa/", "i/registry/", "bubble-game/", "reversi/", "fetch-rss", "reset-db" };
            ...
            foreach (var (ns, sb) in sbs)
            {
                ...
                string className = ns;
                foreach (string path in thirdLevelPaths.Keys)
                {
                    var pathTree = path.Split("/");
                    var subClass = Tools.ConvertToPascalCase(pathTree[1]);
                    if (path.StartsWith(ns, true, null) && !alreadyAppendedClasses.Contains(subClass))
                    {
                ...
                if (!thirdLevelPaths.ContainsKey(ns))
                {
            ...
            foreach (var (path, pathsObject) in thirdLevelPaths)
            {
                ...
                if (!thirdLevelSbsDic.ContainsKey(ns))
                {
                    thirdLevelSbsDic[ns] = new Dictionary<string, StringBuilder>();
                }
